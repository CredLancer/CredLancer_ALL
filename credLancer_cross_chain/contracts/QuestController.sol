...
contract QuestController is Ownable, Pausable, EIP712 ,CCIPReceiver{
  ...
  function _createQuest(
        ...
        if (organizationController.adminOf(orgId) != sender)
            revert Unauthorized();
        if (deadline <= block.timestamp) revert DeadlineAlreadyPassed();
        if (value != reward) revert InvalidValue();
...
function _ccipReceive(
        ...
        uint256 deadline,
        bytes memory signature,
        uint256 nonce) = abi.decode(message.data, (bytes,uint256,uint256,uint256,bytes,uint256));
       if( message.destTokenAmounts[0].token != address(weth)){
        revert("only weth");
       }
        uint256 amount = message.destTokenAmounts[0].amount;
        // temperory workaround because we want to handle eth only for now
        weth.withdraw(amount);
     _createQuest(questCID, reward, orgId, deadline, signature, nonce,sender,amount);
    }
