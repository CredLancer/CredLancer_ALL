...
const getFilteredServiceCondition = (params: IProps) => {
  ...
  if (params.keywordList && params.keywordList.length > 0) {
    keywordFilter = params.keywordList.map(keyword => `{keywords_raw_contains: "${keyword}"}`).join(", ");
  }
...
export const getServices = (params: IProps): Promise<any> => {
  const pagination = params.numberPerPage ? "first: " + params.numberPerPage + ", skip: " + params.offset : "";
  const query = `
    {
  ...
  const query = `
    {
      serviceDescriptionSearchRank(
        ...
        orderBy: id orderDirection: desc ${pagination} ${getFilteredServiceDescriptionCondition(params)}
      ){
        ${serviceDescriptionQueryFields}
        service {
          ${serviceQueryFields}
