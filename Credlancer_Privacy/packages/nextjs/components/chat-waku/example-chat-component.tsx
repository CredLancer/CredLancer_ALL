...
export function WakuChat() {
  /* calling the hook */
  const { messages, handleSendMessage, setSender } = useChat();
    ...
    <div className="max-w-md mx-auto p-4 border rounded-lg shadow-lg">
      <div className="h-64 border rounded-lg overflow-y-scroll">
        ...
        {messages.map((message, index) => (
          <div
            key={index}
            className={`p-2 mb-2 ${message.sender === senderId ? "bg-blue-200" : "bg-gray-200"} rounded-lg`}
          >
      ...
      <div className="mt-4 flex">
        <input
          ...
          className="flex-grow px-2 py-1 rounded-lg border focus:outline-none focus:ring-2 focus:ring-blue-500"
          value={newMessage}
          onChange={e => setNewMessage(e.target.value)}
        />
        <button
          onClick={handleSend}
          className="ml-2 px-4 py-1 bg-blue-500 text-white rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
          Send
        </button>
      </div>
